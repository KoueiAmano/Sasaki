...
ここは AI 主体で開発を進めるプロジェクトの作業場。手動で実装するプロジェクトとは棲み分け、AI 管理対象は `Sasaki/projects/` に内包する。

> このワークスペースは [ait913/Muraki](https://github.com/ait913/Muraki)（友人 Touri の自律開発チーム設計）を土台に、Amano 用に再構成したもの。組織の型・記録の思想は Muraki を踏襲しつつ、団員は外部 CLI (Codex/Gemini) でなく **Claude の Agent (subagent) ツール**で回す。
...
| **Researcher** | `researcher` subagent | 設計**前**のリサーチ (API/メソッド現存確認・一次ソース精読) |
| **Architect** | `architect` subagent | 設計doc執筆 (UI/UX含む技術設計)。実装はしない |
| **Developer** | `developer` subagent | 設計docから実装。テストは書かない |
| **Reviewer** | `reviewer` subagent | 設計docから**テスト生成**→走らせる。コードは見ない |

UI/UXデザインは独立ロール化せず、Architect が設計docの一部として書く。
subagent の定義は `Sasaki/.claude/agents/<role>.md`（このワークスペースに cd すると Claude Code が auto-load する）。各 subagent は召集時に自分の職業的習慣ノート `Sasaki/knowledge/role/<role>.md` を必読。

## ディレクトリ構造

```
Sasaki/                                              # git repo (Amano の自律開発ワークスペース)
...
│   └── role/<role>.md                               # ロール別 職業的習慣ノート (召集時に本人が必読)
...
4. ★ Architect 召集
       設計doc執筆: Sasaki/projects/<slug>/.designs/<YYYYMMDD>-<feature-slug>.md
...
10. Reviewer判定:
       GREEN  → 完了報告 → PR ドラフト
       YELLOW → セカンドオピニオン突合 (「エスカレーション」参照)。一致なら Leader 進行可、割れたら Amano
       RED    → developer 再召集 (or 設計の問題なら architect に戻す)
11. main マージ後、worktree 撤去
```

## エスカレーション (Leader → Amano)

判断の難所は「不安を感じたら」でなく**観測可能なシグナル**で Amano に上げる。Leader のモデルが変わっても発火するよう、シグナルは判断でなくイベントで定義する。形式は AskUserQuestion (要点 + Leader 推奨 + 選択肢 2-4)。
...
- **subagent の肯定的前提も同じ強度で疑う** —「API は健全」「原因はクライアント側」のような範囲を区切る主張は、その外側全部についての否定的主張。誤ると調査方向を丸ごと誤らせる
- **設計が認証・課金・データ削除・破壊的 migration に触れる**
- **プロダクト判断** — 正解が仕様でなく思想で決まるもの (例: 未記録を欠席扱いにするか)

### セカンドオピニオン

岐路の判断 (YELLOW/RED の帰属、設計方針の裁定、否定的主張の受理) では、**独立した subagent を新しい文脈で召集**し（元の判断材料だけを渡し、先の結論は伏せる）同じ問いに答えさせて、Leader の見解と突合する。一致 → 進行。不一致 → Amano。手順書でなくピアレビュー: エキスパート 2 人の意見が割れたら上に上げる、だけ。別モデル (`model` オプション) で撃つと視点が分かれやすい。

### ベースライン失敗の台帳

「既存破損だから無視」は台帳と照合して初めて言える。各 PJ は既知の失敗テストを分類付き (テスト陳腐化 / 環境依存 / **未分類**) で `Sasaki/projects/<slug>/.knowledge/known-failures.md` に持つ。**未分類の失敗を残したままのマージは不可** — 本番バグが「ベースライン失敗の山」に隠れる。台帳が無い PJ は次に触るとき棚卸しして作る。
...
(Architect が責任を持って書く。詳細は `.claude/agents/architect.md` 参照)
...
技術事実は **knowledge の領分**。memory に書くと毎セッション context を食う。memory に書くなら **pointer** に留める:

> Coolify の罠は [[gotcha/coolify-https-redirect-loop]] を見ろ

逆も同様: knowledge に Amano 個人の嗜好を書かない。重複に気付いたら memory 側を縮める。

### knowledge ディレクトリ

```
Sasaki/knowledge/                   # クロスプロジェクト
├── INDEX.md                        # 自動生成
├── library/<topic>.md              # ライブラリ・API知見
├── pattern/<topic>.md              # 設計パターン
├── gotcha/<topic>.md               # ハマりどころ
├── tool-quirk/<topic>.md           # ツール・モデルの癖
└── role/<role>.md                  # ロール別 職業的習慣ノート (INDEX 対象外。召集時に本人が必読、事故後に本人が更新)

Sasaki/projects/<slug>/.knowledge/  # プロジェクト固有 (フラット、frontmatterで category 指定)
└── <topic>.md
```

フォーマット: `Sasaki/knowledge/_TEMPLATE.md`。frontmatter 必須 (`title`, `category`, `project`, `tags`, `created`, `sources`)。本文は `## Context` / `## What` / `## Why` / `## How to apply`。
...
| Leader | tool-quirk | ツール・モデルの癖、判断の経験則 |

追加・編集後は必ず INDEX 再生成:
```
python3 Sasaki/scripts/gen-knowledge-index.py
```
...
**保存先**: `Sasaki/sessions/<yyyy-mm-dd>-<session-short-id>.md`
- `<session-short-id>` = `${CLAUDE_CODE_SESSION_ID:0:8}` (env から取得)
- ターミナルセッションごとに 1 ファイル。並列で複数ターミナルを開いている場合は別ファイルになる
- テンプレート: `Sasaki/sessions/_TEMPLATE.md`
...
- Sasaki 範囲外の作業 (個人開発 PJ など) は **書かない**

## プロジェクトごとの CLAUDE.md (前提知識 + 固有規約)

各プロジェクトの**前提知識・固有規約・主要ワークフロー**は `Sasaki/projects/<slug>/CLAUDE.md` に置く。Claude Code は cwd から上方探索で `CLAUDE.md` を auto-load するので、worktree や project root に cd した瞬間に効く。

### 含める要素 (テンプレ: `Sasaki/projects/_TEMPLATE.md`)

| 要素 | 何を書くか |
|---|---|
| 親規約 link | `親規約: [Sasaki/CLAUDE.md](../../CLAUDE.md)` (機械的に書くだけ。import 不要) |
...
| デプロイ / 外部リソース | URL、app uuid、関連 SKILL など |

### 編集規律 (ライブ書き換え)

プロジェクト状態が変わったら**即時上書き**。「仕様マークダウンの編集規律」が適用される (追記でなく置換、grep→照合→置換→報告)。

### 親規約との関係

- Sasaki/CLAUDE.md = 組織規約 (役割分担・ワークフロー・記録の場所など)
- Sasaki/projects/<slug>/CLAUDE.md = プロジェクト固有規約 + 前提知識 + 主要ワークフロー
- 重複させない。組織規約に書いてあることをプロジェクト側で繰り返さない

## 仕様マークダウンの編集規律

対象: `Sasaki/CLAUDE.md`, `.claude/agents/*.md`, 設計doc, knowledge ファイル。
**方針が変わったら追記でなく置換**する。

### 事故パターン

旧記述: 「A の実装をするときは X の方法で行う」→ 方針転換 → 新記述: 「A の実装をするときは X の方法は絶対に使わない」 ← **追記してしまう**。両方残ると AI は矛盾した指示を読む。

### 編集手順

1. **検索**: 修正対象のキーワードで既存記述を grep (`grep -n "<keyword>" <target.md>`)
...
- ログインが必要なサイトは `Sasaki/scripts/chrome-login.sh [URL]` で同 userDataDir を GUI 起動 → 手動ログイン → 以降 headless でセッション継承
- 設定は `Sasaki/.mcp.json`（`scripts/chrome-devtools-mcp.sh` を呼ぶ）
...
- 設計**後**に必ずユーザー承認ゲート
- 判断の難所はシグナルベースで Amano へ、岐路はセカンドオピニオン突合 (詳細: 「エスカレーション」)
...
新規 AI 管理プロジェクト:
```
mkdir -p Sasaki/projects/<slug>/.designs
git -C Sasaki/projects/<slug> init
cp Sasaki/projects/_TEMPLATE.md Sasaki/projects/<slug>/CLAUDE.md
...
Sasaki ルート自体が git repo。`Sasaki/.gitignore` で以下を除外:
